...
//            }
            ...
            @PutMapping("")
            public ResponseEntity<String> updateProjet (@RequestBody() ProjectDto projectDto, Authentication
            authentication){
            ...
            @PreAuthorize("hasAnyRole('ROLE_ENTREPRENEUR', 'ROLE_ENTREPRENEUR')")
            @DeleteMapping("/{id}")
            public ResponseEntity<String> deleteProjet ( @PathVariable("id") final Long id, Authentication
            authentication){
                final UserDetailsImpl user = (UserDetailsImpl) authentication.getPrincipal();
                Optional<User> userOptional = userRepository.findById(user.getId());
